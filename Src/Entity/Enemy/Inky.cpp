...
// Returns the scatter target position for Inky
sf::Vector2i Inky::GetScatterTargetPosition()
{
	return sf::Vector2i(26, 29);
}

// Returns the chase target position for Inky
sf::Vector2i Inky::GetChaseTargetPosition()
{
	...
	if (gridPos.x + dist.x > NumberOfTilesX - 2)
		finalPos.x = NumberOfTilesX - 2;
	else if (gridPos.x + dist.x < 1)
		finalPos.x = 1;
	else
		finalPos.x = gridPos.x + dist.x;

	if (gridPos.y + dist.y > NumberOfTilesY - 2)
		finalPos.y = NumberOfTilesY - 2;
	else if (gridPos.y + dist.y < 1)
		finalPos.y = 1;
	else
		finalPos.y = gridPos.y + dist.y;

	return finalPos;
}

// Sets up the animations for Inky
